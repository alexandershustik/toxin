...
document.addEventListener('DOMContentLoaded', () => {
	...
	$(document).mouseup(function (e){
		...
		if (!$(".popup__wrap").is(e.target)
			&& $(".popup__wrap").has(e.target).length === 0) {
	...
	$(".service__item").click(function () {
		...
		$(`.service__txt[data-service='${$(this).attr("data-service")}']`).addClass("service__txt-active");
	...
	function buttonActive(){
		if ( $('.popup .popup__input-name').hasClass('popup__input-good') && $('.popup .popup__input-email').hasClass('popup__input-good')) {
			$('.popup .popup__submit').attr("disabled", false);
		}
	}

	function buttonsDisable(){
		if ( $('.popup .popup__input-name').hasClass('popup__input-bad') || $('.popup .popup__input-email').hasClass('popup__input-bad')) {
	...
	function buttonActiveContacts(){
		if ( $('.b-contacts .popup__input-name').hasClass('popup__input-good') && $('.b-contacts .popup__input-email').hasClass('popup__input-good')) {
			$('.b-contacts .popup__submit').attr("disabled", false);
		}
	}

	function buttonsDisableContacts(){
		if ( $('.b-contacts .popup__input-name').hasClass('popup__input-bad') || $('.b-contacts .popup__input-email').hasClass('popup__input-bad')) {
